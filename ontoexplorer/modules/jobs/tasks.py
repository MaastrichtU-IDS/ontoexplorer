...
def check_consistency(version_id: str, ontology_id: str) -> dict:
    ...
    Three scopes are computed:
      - host_only: just the host's named graph (which in OntoExplorer's current
        storage model already includes all transitively-merged owl:imports —
        host_only and host_plus_imports therefore produce the same merge file
        in production)
      - host_plus_imports: nominally the union; in practice identical to
        host_only because of the storage model above
      - host_plus_imports_plus_mireot: same merge + appended MIREOT source
        ontologies fetched via Phase 1's reuse cache

    This is the only scope that materially differs in production — and it's
    the headline test for "does MIREOT-style reuse create cross-import
    contradictions?"

    Long-running (up to 1h hard limit); retries once on failure.
    """
    import json as _json
    import tempfile
    ...
    async def _gather() -> tuple[str, str]:
        async with make_celery_db_session()() as db:
            ...
            return (ont.iri if ont else ""), _graph_iri(ver.ontology_id, version_id)

    t0 = _time.monotonic()
    host_iri, host_graph_iri = asyncio.run(_gather())
    if not host_iri:
    ...
    with tempfile.TemporaryDirectory(prefix=f"consistency_{version_id}_") as tmp:
        report = detect_consistency(
            ...
            import_graph_iris=[],   # see docstring — no separate import graphs in production
            work_dir=Path(tmp),
        )

    r = _get_redis()
    r.setex(consistency_cache_key(version_id), _SEARCH_TTL,
            _json.dumps(_asdict(report)))
